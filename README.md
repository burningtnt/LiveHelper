...
#### LH.Handle.Duplicate(i32(&handle)) -> i32(handle)

创建一个新的句柄 (Handle)，指向同一个资源。
...
#### LH.Handle.Release(i32(handle)) -> void
...
#### LH.Input.GetBuffer(i32, i32, i32) -> i32
...
必须导出函数 `LH.Technique.EntryPoint() -> i32(handle, clip)` 作为入口点：

```
LH.Technique.EntryPoint(
) -> i32(handle, clip) // 摄像机配置。 
```

拥有以下特别的 API：

#### 预定义输入 (Input) `lh.progress` / `GetF32`

当前程序的进度: [0, 1) 间浮点数

#### LH.Technique.MakeClip(f32, f32, f32, f32, f32) -> i32(handle, clip)

创建一个摄像机配置供 LiveHelper 渲染。

```
LH.Technique.MakeClip(
    ...
    [IN] f32 rZ,
    [IN] f32 rW,
) -> i32(handle, clip)  // 摄像机配置，请在 LH.Technique.EntryPoint 中返回。
```
...
必须导出函数 `LH.Manager.EntryPoint() -> i32(handle, clip)` 作为入口点：

```
LH.Manager.EntryPoint(
) -> i32(handle, clip) // 摄像机配置。 
```

拥有以下特别的 API：

#### 预定义输入 (Input) `lh.camera_counts` / `GetF32`

可支配的分镜头 (Clip) 个数：正整数

#### 预定义输入 (Input) `lh.camera.$id.duration_ms` / `GetF32`
...
```
LH.Manager.Render.ActivateSingle(
   i32(unsigned) cameraIndex      // 摄像机下标 (0-based)
) -> i32(handle, render_request)  // 帧渲染配置，LiveHelper 渲染该分镜头 (Clip)
```

#### LH.Manager.Render.Mix(i32(handle, render_request), i32(handle, render_request), f32) -> i32(handle, render_request)

使 LiveHelper 同时激活两个分镜头 (Clip) 并线性插值合并。

```
LH.Manager.Render.Mix(
