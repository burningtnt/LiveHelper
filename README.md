...
必须导出函数 `Technique.EntryPoint() -> i32(handle, clip)` 作为入口点：
...
#### Technique.MakeClip(f32, f32, f32, f32, f32) -> i32(handle, clip)
...
Technique.MakeClip(
    ...
    [IN] f32 rW,
    [IN] f32 fov,         // FOV
    [IN] i32 renderHUD,   // 若为 0，不渲染 HUD；否则，渲染 HUD。
    [IN] i32 renderHand,  // 若为 0，不渲染玩家的手；否则，渲染玩家的手。
) -> i32(handle, clip)    // 摄像机配置，请在 Technique.EntryPoint 中返回。
```
...
必须导出函数 `Manager.EntryPoint() -> i32(handle, clip)` 作为入口点：
...
#### 预定义输入 (Input) `clip.$id.duration_ms` / `GetF32`
...
#### Manager.Render.Single(i32(unsigned)) -> i32(handle, render_request)

使 LiveHelper 激活一个特定的分镜头 (Clip)。

```
Manager.Render.ActivateSingle(
