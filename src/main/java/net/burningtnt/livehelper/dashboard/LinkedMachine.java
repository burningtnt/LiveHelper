...
import net.burningtnt.livehelper.components.InputDeclaration;
...
//                            .withMemoryFactory() TODO: Replace with LWJGL-based memory access to reduce runtime index validation overload
                ...
                .build();
        this.entrypoint = this.script.export(switch (usage) {
            case CLIP -> "Technique.EntryPoint";
            case MANAGER -> "Manager.EntryPoint";
        });
    ...
    /* package-private */ <T> T executeProgram(Class<T> clazz) throws ComponentException {
        long[] result = this.entrypoint.apply();
        if (result == null || result.length != 1) {
            throw new ChicoryException("Invalid return value: " + Arrays.toString(result));
        }

        int i = Math.toIntExact(result[0]);
        return clazz.cast(resource().remove(i));
    }

    /* package-private */ Map<String, InputValue> inputs() {
        return this.inputs;
    }

    /* package-private */ Int2ObjectMap<Object> resource() {
